...
if(octomap_builder_CONFIG_INCLUDED)
  return()
endif()
set(octomap_builder_CONFIG_INCLUDED TRUE)

# set variables for source/devel/install prefixes
if("TRUE" STREQUAL "TRUE")
  set(octomap_builder_SOURCE_PREFIX /home/greg/fetch_ws/src/fetchit/octomap_builder)
  set(octomap_builder_DEVEL_PREFIX /home/greg/fetch_ws/src/fetchit/octomap_builder/cmake-build-debug/devel)
  set(octomap_builder_INSTALL_PREFIX "")
  set(octomap_builder_PREFIX ${octomap_builder_DEVEL_PREFIX})
else()
  set(octomap_builder_SOURCE_PREFIX "")
  set(octomap_builder_DEVEL_PREFIX "")
  set(octomap_builder_INSTALL_PREFIX /usr/local)
  set(octomap_builder_PREFIX ${octomap_builder_INSTALL_PREFIX})
endif()

# warn when using a deprecated package
if(NOT "" STREQUAL "")
  set(_msg "WARNING: package 'octomap_builder' is deprecated")
...
foreach(library ${libraries})
  ...
  elseif(IS_ABSOLUTE ${library})
    ...
    foreach(path /home/greg/fetch_ws/src/fetchit/octomap_builder/cmake-build-debug/devel/lib;/home/greg/fetch_ws/devel/lib;/opt/ros/melodic/lib)
    ...
    endforeach()
    if(lib)
      _list_append_unique(octomap_builder_LIBRARY_DIRS ${lib_path})
      list(APPEND octomap_builder_LIBRARIES ${lib})
    else()
      # as a fall back for non-catkin libraries try to search globally
      find_library(lib ${library})
      if(NOT lib)
        message(FATAL_ERROR "Project '${PROJECT_NAME}' tried to find library '${library}'.  The library is neither a target nor built/installed properly.  Did you compile project 'octomap_builder'?  Did you find_package() it before the subdirectory containing its code is included?")
      endif()
      list(APPEND octomap_builder_LIBRARIES ${lib})
    endif()
  endif()
...
set(octomap_builder_EXPORTED_TARGETS "")
# create dummy targets for exported code generation targets to make life of users easier
foreach(t ${octomap_builder_EXPORTED_TARGETS})
  if(NOT TARGET ${t})
    add_custom_target(${t})
  endif()
endforeach()

set(depends "pcl_ros;roscpp")
foreach(depend ${depends})
  string(REPLACE " " ";" depend_list ${depend})
  # the package name of the dependency must be kept in a unique variable so that it is not overwritten in recursive calls
  list(GET depend_list 0 octomap_builder_dep)
  list(LENGTH depend_list count)
  if(${count} EQUAL 1)
    # simple dependencies must only be find_package()-ed once
    if(NOT ${octomap_builder_dep}_FOUND)
      find_package(${octomap_builder_dep} REQUIRED NO_MODULE)
    endif()
  else()
    # dependencies with components must be find_package()-ed again
    list(REMOVE_AT depend_list 0)
    find_package(${octomap_builder_dep} REQUIRED NO_MODULE ${depend_list})
  endif()
  _list_append_unique(octomap_builder_INCLUDE_DIRS ${${octomap_builder_dep}_INCLUDE_DIRS})

  # merge build configuration keywords with library names to correctly deduplicate
  _pack_libraries_with_build_configuration(octomap_builder_LIBRARIES ${octomap_builder_LIBRARIES})
  _pack_libraries_with_build_configuration(_libraries ${${octomap_builder_dep}_LIBRARIES})
  _list_append_deduplicate(octomap_builder_LIBRARIES ${_libraries})
  # undo build configuration keyword merging after deduplication
  _unpack_libraries_with_build_configuration(octomap_builder_LIBRARIES ${octomap_builder_LIBRARIES})

  _list_append_unique(octomap_builder_LIBRARY_DIRS ${${octomap_builder_dep}_LIBRARY_DIRS})
  list(APPEND octomap_builder_EXPORTED_TARGETS ${${octomap_builder_dep}_EXPORTED_TARGETS})
endforeach()

set(pkg_cfg_extras "")
foreach(extra ${pkg_cfg_extras})
  if(NOT IS_ABSOLUTE ${extra})
    set(extra ${octomap_builder_DIR}/${extra})
  endif()
  include(${extra})
endforeach()
